---
outline: [2, 3]
description: Troubleshooting common issues with llama-cpp-node
---
# Troubleshooting
## ESM Usage
`llama-cpp-node` is an [ES module](https://nodejs.org/api/esm.html#modules-ecmascript-modules), so can only use `import` to load it and cannot use [`require`](https://nodejs.org/docs/latest-v18.x/api/esm.html#require:~:text=Using%20require%20to%20load%20an%20ES%20module%20is%20not%20supported%20because%20ES%20modules%20have%20asynchronous%20execution.%20Instead%2C%20use%20import()%20to%20load%20an%20ES%20module%20from%20a%20CommonJS%20module.).
...
### Using in CommonJS
If you cannot use ESM in your project, you can still use the `import` function from a CommonJS module to load `llama-cpp-node`:
```typescript
async function myLogic() {
    const {getLlama} = await import("llama-cpp-node");
}
...
you can use this workaround to `import` `llama-cpp-node`:
```typescript
async function myLogic() {
    const nlc: typeof import("llama-cpp-node") = await Function('return import("llama-cpp-node")')();
    const {getLlama} = nlc;
...
To allow `llama-cpp-node` to build the binaries, install the required packages first:
```bash
pkg update
...
The solution to this issue is to remove these settings to let `llama-cpp-node` find the optimal configuration that works on your machine
...
`llama-cpp-node` has a built-in memory estimation mechanism that estimates the memory required for the model to run on the GPU in order to find the optimal configuration to load a model with and create a context with.
...
```shell
npx --no llama-cpp-node inspect measure [modelPath]
```

To work around this issue, you can force `llama-cpp-node` to ignore the memory safeguards and load the model anyway by setting the `ignoreMemorySafetyChecks` options to `true`:
```typescript
import {fileURLToPath} from "url";
import path from "path";
import {getLlama, LlamaChatSession} from "llama-cpp-node";
