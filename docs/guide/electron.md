---
description: Using llama-cpp-node in Electron applications
---
# Using in Electron
`llama-cpp-node` is fully supported in [Electron](https://www.electronjs.org), and also includes custom Electron-specific adaptations.

You can only use `llama-cpp-node` on the main process in Electron applications.
Trying to use `llama-cpp-node` on a renderer process will crash the application.

You can scaffold an example Electron app that uses `llama-cpp-node` with complete configuration for packaging and distribution by running the following command:
```shell
npm create llama-cpp-node@latest -- --template electron-typescript-react
```

::: tip
Even if you intend to integrate `llama-cpp-node` into your existing Electron app,
it's still recommended that you scaffold a new Electron project and investigate the `electron-builder.ts` file
to see how to configure your existing Electron app to work well with `llama-cpp-node`.
:::

## Electron Support
In Electron, when there's no binary available for the current platform,
`llama-cpp-node` won't build from source by default,
...
so that `llama-cpp-node` can find them.
...
ensure that `llama-cpp-node` is not bundled, and is instead treated as an external module.

Marking `llama-cpp-node` as an external module will prevent its code from being bundled with your application code,
and instead, it'll be loaded from the `node_modules` directory at runtime (which should be packed into a `.asar` archive).

The file structure of `llama-cpp-node` is crucial for it to function correctly,
so bundling it will break its functionality.
Moreover, since `llama-cpp-node` includes prebuilt binaries (and also local builds from source),
...
Using ASAR is the recommended way to bundle `llama-cpp-node` in your Electron app.
